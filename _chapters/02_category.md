...
Now that we have come up with a definition of a set *element* using just functions, we can try to draw the elements of our set as an external diagram.

![Functions from the singleton set](../02_category/elements_singleton_functions_partly_external.svg)

However, our diagram is not yet fully external, as it depends on the idea of the singleton set, i.e. the set with one *element*. Furthermore, this makes the whole definition circular, as we have to already define the concept of an element in order to derive the concept of an one-element set.
...
By switching to external diagrams, we lose sight of the particular elements of our sets, but we gain the ability to zoom out and see the big picture. In the same way that the whole realm of sets can be thought as one category, a programming language can also be thought of as a category. The concept of a category allows us to find and analyze similarities between these and other structures.

**NB:** The word "Object" appears in both programming languages and in category theory, but has completely different meanings. A categorical object is equivalent to a *type* or a *class* in programming language theory.

Sets Vs Categories
---

One remark before we continue: in the last few paragraphs, it might sound as if though category theory and set theory were somehow competing with each another. Perhaps that notion would be somewhat correct if category and set theory were meant to describe *concrete* phenomena, in the way that the theory of relativity and the theory of quantum mechanics are both supposed to explain the physical world. Concrete theories are conceived mainly as *descriptions* of the world, and as such it makes sense for them to be connected together in some sort of hierarchy. 
...
A category consists of a collection of objects as well as some arrows connecting some of them to others. We call the arrows *morphisms*.
...
This particular analogy (a set as a category with no morphisms) is, however, not very useful. Not because it's incorrect in any way , but rather because category theory is *all about the morphisms*. If an arrow in set theory is nothing but a connection between a source and a destination, in category theory the *objects* are nothing but a source and destination for the arrows that connect them to other objects. This is why, in the diagram above, the arrows, and not the objects, are colored: if you ask me, the category of sets should really be called *the category of functions*.

Speaking of which, note that objects in a category can be connected together by multiple arrows and that two arrows having the same source and target sets does not in any way make them equivalent (it does not actually mean that they would produce the same value).
...
Commutativity law is applicable only in contexts where the order is irrelevant i.e. when an object can be represented as the sum of its parts and therefore *can be combined in whichever way*. But there are many cases in which an object should be represented by the sum of its parts, but only when *combined in one specific way*.
